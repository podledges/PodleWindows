...
Before printing, the nudge wrapper reads `state/.lock` and walks at most eight parents from its own pid in its own separate, hard-coded loop, independent of `bin/fm-lock.sh`'s ancestry walk (`fm_harness_ancestry_pid()` in `bin/fm-session-lock-lib.sh`, which now walks up to sixteen parents, can extend past a claude-named match to a still-more-ancestral one, and skips the shared `claude daemon run` background-session host when choosing the pid to record) and of Pi's `lockOwnership()`.
